...
class Color {
    ...
    public static function rgbToHex( $rgb = array() ){
        ...
        // Convert RGB to HEX
        $hex[0] = self::twoNumbers (dechex( $rgb['R'] ));
        $hex[1] = self::twoNumbers (dechex( $rgb['G'] ));
        $hex[2] = self::twoNumbers (dechex( $rgb['B'] ));

        return implode( '', $hex );

  }
    protected static function twoNumbers($item)
    {
        if (strlen($item)==1) {
            $item.=$item;
        }
    ...
    public function darken( $amount = self::DEFAULT_ADJUST ){
        ...
        $this->load(self::hslToHex($darkerHSL));
        // Return as HEX
