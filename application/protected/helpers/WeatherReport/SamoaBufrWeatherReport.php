...
/**
 * 
 * Here you are able to extend BufrWeatherReport if it is going to have some
 * changes specific for Samoa only.
 *
 */

class SamoaBufrWeatherReport extends Template307080BufrWeatherReport
{
    /**
     * 
     */
    public function prepareSection4()
	{
		...
		{
            $tmp = str_pad(decbin(ord($station_display_name[$j])), 8, '0', STR_PAD_LEFT);
            $tmp_str .= $tmp;
            $tmp_str_2 .= ' '.$tmp;
        }
        ...
        $this->explanations[$s][$i][] = '301090 -> 301011 -> 004001: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_year.'</span> (Year, 12 bits)';   
        $i++;   
        ...
        if (isset($this->sensors['pressure_height']))
		{
        ...
        if (isset($this->sensors['pressure']))
		{
			...
			else
			{
                ...
                $str_tmp = round($tmp/10, 0); // apply scale & reference value
		...
		{
			//// we store MSL in hPa. Need to conver it to Pa
        ...
        if (isset($this->sensors['pressure']) && isset($sensors_3hr_ago['pressure']))
		{
			...
			else
			{
                ...
                $tmp3 = round(($pressure_now - $pressure_3hr)/10, 0); // apply scale
                $str_tmp = abs($tmp3) + 500; // get abs value & apply ref
        ...
        if (isset($this->sensors['pressure']) && isset($sensors_24hr_ago['pressure']))
		{
			...
			else 
			{
                ...
                $tmp3 = round(($pressure_now - $pressure_24hr)/10, 0); // apply scale
                $str_tmp = abs($tmp3) - (-1000); // get abs value & apply ref
		...
		{
            $tmp = It::convertMetric($this->calculations['DewPoint']['value'], 'celsius', 'kelvin'); // we store dewpoint in celsius degree.
            $str_tmp = round($tmp*100);
        ...
        if (isset($this->sensors['humidity']))
		{
			...
			{
                $this->report_parts[$s][$i] = $this->prepare_binary_value(round($this->sensors['humidity']['sensor_feature_value']), 7); //decbin(round($this->sensors['humidity']['sensor_feature_value']));
        ...
        if (isset($this->sensors['visibility_1']))
		{
			...
			else 
			{
                ...
                $tmp2 = round($tmp / 10, 0); // apply scale & ref
                
				$this->report_parts[$s][$i] = $this->prepare_binary_value($tmp2, 13);
                $expl_val = '<span>'.$this->sensors['visibility_1']['sensor_feature_value'].' '.$this->sensors['visibility_1']['metric_code'].' = '.$tmp.' meter. Apply scale=-1 => '.$tmp2.' </span>';
            }
        } 
		else 
		{
        ...
        if (isset($this->sensors['rain_in_period']))
		{
			...
			else
			{
				...
				// get total rain value between script_runtime and 2hr ago
                $rain_last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['rain_in_period']['sensor_feature_id'], $script_runtime, 24, false);

                $tmp = It::convertMetric($rain_last_24_hr, $this->sensors['rain_in_period']['metric_code'], 'millimeter');

                $tmp_str = round($tmp * 10) - (-1); // apply scale & ref
        ...
        $this->report_parts[$s][$i] = $this->prepare_binary_value(3, 8); //'00000011';
		...
		// 4 bits. Metric = code table. Scale = 0. Ref = 0.	 
        if (isset($this->sensors['cloud_amount_height_1']))
		{
            if ($this->sensors['cloud_amount_height_1']['is_m'])
			{
                $this->report_parts[$s][$i] = '1111';
			...
			{
                $tmp1 = $this->_get_cloud_cover_code(round($this->sensors['cloud_amount_height_1']['sensor_feature_value'],1));
                $this->report_parts[$s][$i] = $this->prepare_binary_value($tmp1, 4);
                $expl_val = '<span>'.$tmp1.' ('.round($this->sensors['cloud_amount_height_1']['sensor_feature_value']).'/8)</span>';
            }
        } 
		else 
		{
		...
		// 11 bits. Metric = m. Scale = -1. Ref = -40. 
        if (isset($this->sensors['cloud_height_height_1']))
		{
            if ($this->sensors['cloud_height_height_1']['is_m'])
			{
                $this->report_parts[$s][$i] = '11111111111';
			...
			{
                $tmp1 = It::convertMetric($this->sensors['cloud_height_height_1']['sensor_feature_value'], $this->sensors['cloud_height_height_1']['metric_code'], 'meter');
				
				// 1. apply scale = -1 (value * 10^-1 )
				// 2. round to get integer			
				// 3. apply ref = -40				
                $tmp2 = round($tmp1/10 - (-40));
                $this->report_parts[$s][$i] = $this->prepare_binary_value($tmp2, 11);
                $expl_val = '<span>'.round($this->sensors['cloud_height_height_1']['sensor_feature_value']).' '. $this->sensors['cloud_height_height_1']['metric_code'].' = '.round($tmp1,1).' meter.</span> With scale=-1 and ref=40: <span>'.$tmp2.'</span>';
            }
        } 
		else 
		{
		...
		// 6 bits. Metric = code table. Scale = 0. Ref = 0.			
        $this->report_parts[$s][$i] = '111111';
        $this->explanations[$s][$i][] = '302035 -> 302005 -> 008002 (Repetition #2): <span>'.$this->report_parts[$s][$i].'</span> => <span>Missing (No such measurement)</span> (Vertical significance (surface observations), 6bits)';   
        $i++;          
        
		// 302035 -> 302005 -> 020011 (Repetition #2): Cloud amount
		// 4 bits. Metric = code table. Scale = 0. Ref = 0.			
        if (isset($this->sensors['cloud_amount_height_2']))
		{
            if ($this->sensors['cloud_amount_height_2']['is_m'])
			{
                $this->report_parts[$s][$i] = '1111';
                $expl_val = '<span>Last data about Cloud Amount is unavailable</span>';                
            }
			...
			{
                $tmp1 = $this->_get_cloud_cover_code(round($this->sensors['cloud_amount_height_2']['sensor_feature_value'],1));
                $this->report_parts[$s][$i] = $this->prepare_binary_value($tmp1, 4);
                $expl_val = '<span>'.$tmp1.' ('.round($this->sensors['cloud_amount_height_2']['sensor_feature_value']).'/8)</span>';
            }
        }
		else 
		{
		...
		// 11 bits. Metric = m. Scale = -1. Ref = -40. 		
        if (isset($this->sensors['cloud_height_height_2']))
		{
            if ($this->sensors['cloud_height_height_2']['is_m'])
			{
                $this->report_parts[$s][$i] = '11111111111';
                $expl_val = '<span>Last data about Cloud Height is unavailable</span>';
            }
			...
			{
                $tmp1 = It::convertMetric($this->sensors['cloud_height_height_2']['sensor_feature_value'], $this->sensors['cloud_height_height_2']['metric_code'], 'meter');
                ...
                $tmp2 = round($tmp1/10)-(-40);
                $this->report_parts[$s][$i] = $this->prepare_binary_value($tmp2, 11);
                $expl_val = '<span>'.round($this->sensors['cloud_height_height_2']['sensor_feature_value']).' '. $this->sensors['cloud_height_height_2']['metric_code'].' = '.round($tmp1,1).' meter.</span> With scale=-1 and ref=40: <span>'.$tmp2.'</span>';
            }
        } 
		else 
		{
		...
		// 6 bits. Metric = code table. Scale = 0. Ref = 0.			
        $this->report_parts[$s][$i] = '111111';
        $this->explanations[$s][$i][] = '302035 -> 302005 -> 008002 (Repetition #3): <span>'.$this->report_parts[$s][$i].'</span> => <span>Missing (No such measurement)</span> (Vertical significance (surface observations), 6bits)';   
        $i++;          
        
		// 302035 -> 302005 -> 020011 (Repetition #3): Cloud amount
		// 4 bits. Metric = code table. Scale = 0. Ref = 0.			
        if (isset($this->sensors['cloud_amount_height_3']))
		{
            if ($this->sensors['cloud_amount_height_3']['is_m'])
			{
                $this->report_parts[$s][$i] = '1111';
                $expl_val = '<span>Last data about Cloud Amount is unavailable</span>';                
            }
			...
			{
                $tmp1 = $this->_get_cloud_cover_code(round($this->sensors['cloud_amount_height_3']['sensor_feature_value'],1));
                $this->report_parts[$s][$i] = $this->prepare_binary_value($tmp1, 4);
                $expl_val = '<span>'.$tmp1.' ('.round($this->sensors['cloud_amount_height_3']['sensor_feature_value']).'/8)</span>';
            }
        } 
		else 
		{
		...
		// 11 bits. Metric = m. Scale = -1. Ref = -40. 			
        if (isset($this->sensors['cloud_height_height_3']))
		{
            if ($this->sensors['cloud_height_height_3']['is_m'])
			{
                $this->report_parts[$s][$i] = '11111111111';
                $expl_val = '<span>Last data about Cloud Height is unavailable</span>';
            }
			...
			{
                $tmp1 = It::convertMetric($this->sensors['cloud_height_height_3']['sensor_feature_value'], $this->sensors['cloud_height_height_3']['metric_code'], 'meter');
                ...
                $tmp2 = round($tmp1/10)-(-40);
                $this->report_parts[$s][$i] = $this->prepare_binary_value($tmp2, 11);
                $expl_val = '<span>'.round($this->sensors['cloud_height_height_3']['sensor_feature_value']).' '. $this->sensors['cloud_height_height_3']['metric_code'].' = '.round($tmp1,1).' meter.</span> With scale=-1 and ref=40: <span>'.$tmp2.'</span>';
            }
        } 
		else 
		{
        ...
        $this->report_parts[$s][$i] = $this->prepare_binary_value(1, 8);//'00000001';
		...
		// 12 bits.  Metric = Hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = '011111111111';
        ...
        if (isset($this->sensors['sun_duration_in_period']) && isset($this->sensors['sun_duration_in_period']['sensor_feature_period']))
		{
		...
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048.
        $this->report_parts[$s][$i] = ($script_runtime_hour == '11' && $script_runtime_minute == '00') ? '011111101000' : '111111111111';
        $this->explanations[$s][$i][] = '302043 -> 302039 -> 004024 (Repetition #2): <span>'.$this->report_parts[$s][$i].'</span> => <span>'.( ($script_runtime_hour == '11' && $script_runtime_minute == '00') ? '(-24) - (-2048) = 2024' : 'Missing, Available only for 11UTC' ).'</span> (Time period of sunshine measuring, hrs, 12bits)';   
        $i++;  
        ...
        if (isset($this->sensors['sun_duration_in_period']) && isset($this->sensors['sun_duration_in_period']['sensor_feature_period']))
		{
            if ($script_runtime_hour == '11' && $script_runtime_minute == '00')
			{
				// get object of SunDuration handler
                $handler_obj = SensorHandler::create($this->sensors['sun_duration_in_period']['handler_id_code']);
                
				// get total sunshine in last 24 hours
                $last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['sun_duration_in_period']['sensor_feature_id'], $script_runtime, 24, false);
			...
			{
                $this->report_parts[$s][$i] = '11111111111';
                $this->explanations[$s][$i][] = '<span>It is not 11UTC. Total 24h Sunshine is calculated only at 11:00UTC</span>';                   
            }
        } 
		else
		{
		...
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048.
        $this->report_parts[$s][$i] = '011111111111';
        $this->explanations[$s][$i][] = '302043 -> 302040 -> 004024 (Repetition #1): <span>'.$this->report_parts[$s][$i].'</span> => <span>(-1)-(-2048) = 2047</span> (Time period or displacement, Hours, 12bits)';   
        $i++;          

		// 302043 -> 302040 -> 013011 (Repetition #1): Total rain for 1 hour
		// 14 bits. Metric = kg/m2. Scale = 1. Ref = -1.
        if (isset($this->sensors['rain_in_period']) && isset($this->sensors['rain_in_period']['sensor_feature_period']))
		{
            if ($this->sensors['rain_in_period']['is_m'])
			{
                $this->report_parts[$s][$i] = '11111111111111';
        ...
        if (isset($this->sensors['rain_in_period']))
		{
			...
			else 
			{
                ...
                if ($rain_total_hours == 24)
				{
					$rain_last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['rain_in_period']['sensor_feature_id'], $script_runtime, 24, false);
                } 
				else 
				{
					$rain_last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['rain_in_period']['sensor_feature_id'], $script_runtime, 6, false);
                }
        ...
        if (isset($this->sensors['temperature']) && ($script_runtime_minute == '00') && in_array($script_runtime_hour, array('00', '12'))) 
		{
            $handler_obj = SensorHandler::create($this->sensors['temperature']['handler_id_code']);
            
			if ($script_runtime_hour == '12')
			{
		...
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = (($script_runtime_hour == '12' && $script_runtime_minute == '00') ? '011111100100' : '111111111111');
        $this->explanations[$s][$i][] = '302043 -> 302041 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.(($script_runtime_hour == '12' && $script_runtime_minute == '00') ? '(-28) - (-2048) = 2020' : 'Missed because it is not at 12 UTC').'</span> (Time period of temperature measuring, hours, 12bits)';   
        $i++;          
        
		// 302043 -> 302041 -> 004024: Time period of temperature measuring
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = (($script_runtime_hour == '12' && $script_runtime_minute == '00') ? '011111111100' : '111111111111');
        $this->explanations[$s][$i][] = '302043 -> 302041 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.(($script_runtime_hour == '12' && $script_runtime_minute == '00') ? '(-4) - (-2048) = 2044' : 'Missed because it is not at 12 UTC').'</span> (Time period or replacement, 12bits)';         
        $i++;
        ...
        if (isset($this->sensors['temperature']))
		{
            if ($script_runtime_hour == '12' && $script_runtime_minute == '00')
			{
			...
			{
                $this->report_parts[$s][$i] = '1111111111111111';
                $expl_val = 'MAX tempr can be calculated only at 12UTC';                
            }
        } 
		else 
		{
		...
		// 302043 -> 302041 -> 004024: Time period of temperature measuring
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = (($script_runtime_hour == '00' && $script_runtime_minute == '00') ? '011111100100' : '111111111111');
        $this->explanations[$s][$i][] = '302043 -> 302041 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.(($script_runtime_hour == '00' && $script_runtime_minute == '00') ? '-28 - (-2048) = 2020' : 'Missed because it is not at 00 UTC').'</span> (Time period of temperature measuring, 12bits)';   
        $i++;          
        
		// 302043 -> 302041 -> 004024: Time period of temperature measuring
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = (($script_runtime_hour == '00' && $script_runtime_minute == '00') ? '011111111100' : '111111111111');
        $this->explanations[$s][$i][] = '302043 -> 302041 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.(($script_runtime_hour == '00' && $script_runtime_minute == '00') ? '-4 - (-2048) = 2044' : 'Missed because it is not at 00 UTC').'</span> (Time period or displacement, 12bits)';         
        $i++;
        ...
        if (isset($this->sensors['temperature']))
		{
            if ($script_runtime_hour == '00' && $script_runtime_minute == '00')
			{
        ...
        if (isset($this->sensors['solar_radiation_in_period']) && isset($this->sensors['solar_radiation_in_period']['sensor_feature_period']))
		{
			...
			else 
			{
                ...
                $last_1_hr = $handler_obj->getTotalInLastXHr($this->sensors['solar_radiation_in_period']['sensor_feature_id'], $script_runtime, 1, false);
		...
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = ($script_runtime_hour == '00' && $script_runtime_minute == '00') ?'011111101000' : '111111111111';
        $this->explanations[$s][$i][] = '302045 -> 004024 (Repetition #2): <span>'.$this->report_parts[$s][$i].'</span> => <span>'.( ($script_runtime_hour == '00' && $script_runtime_minute == '00') ? '(-24) - (-2048) = 2024' : 'Missing, It is not 00UTC' ).'</span> (Time period or displacement, Hour, 12 bits)';   
        $i++;
        ...
        if (isset($this->sensors['solar_radiation_in_period']))
		{
            if ($script_runtime_hour == '00' && $script_runtime_minute == '00')
			{
                ...
                $last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['solar_radiation_in_period']['sensor_feature_id'], $script_runtime, 24, false);
			...
			{
                $this->report_parts[$s][$i] = '11111111111111111111';
                $expl_val = '24hr radiation can be calculated only at 00UTC';                 
            }
        } 
		else 
		{
        ...
        $this->report_parts[$s][$i] = '111111';
        $this->explanations[$s][$i][] = '302046 -> 012049: <span>'.$this->report_parts[$s][$i].'</span> => <span>Missing</spam> (Temperature change over period specified, K, 6 bits)';
        $i++;   
