...
/*
 * Extension of WeatherReport. Has its own implemetation of generate() function.
 * BUFR is a data transmission format, a continuous binary stream comprising of 6 sections. 
 * We use a standard BUFR format known as "307080". This has section: 0, 1, 3, 4, 5. 
 * It does not have section 2. Objective is to construct & output data, which 
 * was collected from (a) weather station(s) into a binary string of data 
 * formatted according to BUFR template 207080. We are constructing BUFR reports 
 * one station at a time.
 * 
 * Sections in 307080
 * Section 0: indicator section
 * Section 1: identification section => length & identification of the message
 * Section 3: data description => 307080
 * Section 4: data
 * Section 5: end => 7777
 * Sections 0,1,3,5 have a series of octets (8 bits). 
 * Section 4 has a few octets at the beginning and it has sequence of 
 * descriptors (according to template 3 07 080. 
 * 
 * All BUFR codes, tables and references: WMO_306_Vol I.2_en.pdf (\input_docs\BUFR & SYNOP)
 * 
 */
class Template307080BufrWeatherReport extends BaseBufrWeatherReport
{
	protected function getTotalNumberOfOctets()
	{
		return 8 + 23 + 9 + 195 + 4;
	}
	
    public function prepareSection1()
	{
		parent::prepareSection1();
        ...
        $this->report_parts[$s][$i] = $this->prepare_binary_value(23, 24); 
        $this->explanations[$s][$i][] = 'Octets 1-3: <span>'.$this->report_parts[$s][$i].'</span> => <span>23</span> (24 bits)';   
        $i++;
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8); 
        $this->explanations[$s][$i][] = 'Octet 4: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Standard WMO Tables, 8 bits)';   
        $i++;  
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($this->station_info->wmo_originating_centre, 16); 
        $this->explanations[$s][$i][] = 'Octets 5-6: <span>'.$this->report_parts[$s][$i].'</span> => <span>'. $this->station_info->wmo_originating_centre .'</span> (Originating Center, 16 bits)';   
        $i++;
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 16); 
        $this->explanations[$s][$i][] = 'Octets 7-8: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (No subcenter, 16 bits)';   
        $i++;        
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8); 
        $this->explanations[$s][$i][] = 'Octet 9: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Fixed, 8 bits)';   
        $i++;  
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8); 
        $this->explanations[$s][$i][] = 'Octet 10: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Fixed, 8 bits)';   
        $i++;  
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8);
        $this->explanations[$s][$i][] = 'Octet 11: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Fixed, 8 bits)';   
        $i++;       
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($this->getSubCategory($script_runtime_hour), 8);
        $this->explanations[$s][$i][] = 'Octet 12: <span>'.$this->report_parts[$s][$i].'</span> => <span>'. $this->report_parts[$s][$i] .'</span> (Fixed, 8 bits)';   
        $i++;
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8);
        $this->explanations[$s][$i][] = 'Octet 13: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Fixed, 8 bits)';   
        $i++;
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(14, 8);
        $this->explanations[$s][$i][] = 'Octet 14: <span>'.$this->report_parts[$s][$i].'</span> => <span>14</span> (Version implemented on 4 Nov 2009, 8 bits)';   
        $i++;
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8);
        $this->explanations[$s][$i][] = 'Octet 15: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Fixed, 8 bits)';   
        $i++;   
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($script_runtime_year, 16);
        $this->explanations[$s][$i][] = 'Octets 16-17: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_year.'</span> (Year, 16 bits)';   
        $i++;   
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($script_runtime_month, 8);
        $this->explanations[$s][$i][] = 'Octet 18: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_month.'</span> (Month, 8 bits)';   
        $i++;        
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($script_runtime_day, 8);
        $this->explanations[$s][$i][] = 'Octet 19: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_day.'</span> (Day, 8 bits)';   
        $i++;         
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($script_runtime_hour, 8);
        $this->explanations[$s][$i][] = 'Octet 20: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_hour.'</span> (Hour, 8 bits)';   
        $i++;         
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($script_runtime_minute, 8);
        $this->explanations[$s][$i][] = 'Octet 21: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_minute.'</span> (Minute, 8 bits)';   
        $i++;     
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($script_runtime_second, 8);
        $this->explanations[$s][$i][] = 'Octet 22: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$script_runtime_second.'</span> (Second, 8 bits)';   
        $i++;  
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8);
        $this->explanations[$s][$i][] = 'Octet 23: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Reserved, 8 bits)';         
    }
    
    public function prepareSection3()
	{
		parent::prepareSection3();
        ...
        $this->report_parts[$s][$i] = $this->prepare_binary_value(9, 24);
        $this->explanations[$s][$i][] = 'Octets 1-3: <span>'.$this->report_parts[$s][$i].'</span> => <span>9</span> (length of section, 24 bits)';   
        $i++;   
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(0, 8);
        $this->explanations[$s][$i][] = 'Octet 4: <span>'.$this->report_parts[$s][$i].'</span> => <span>0</span> (Fixed, 8 bits)';   
        $i++;   
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value(1, 16);
        $this->explanations[$s][$i][] = 'Octets 5-6: <span>'.$this->report_parts[$s][$i].'</span> => <span>1</span> (Number of datasets, 16 bits)';   
        $i++;   
        
        $this->report_parts[$s][$i] = '10000000';
        $this->explanations[$s][$i][] = 'Octet 7: <span>'.$this->report_parts[$s][$i].'</span> => <span>1st bit=1, rest=0</span> (fixed, 8 bits)';   
        $i++;   
       
        $this->report_parts[$s][$i] = '1100011101010000';
        $this->explanations[$s][$i][] = 'Octets 8-9: <span>'.$this->report_parts[$s][$i].'</span> => <span>Template 3 07 080</span> (16 bits)';   
    }
    
    public function prepareSection4()
	{
		parent::prepareSection4();
        ...
        $this->report_parts[$s][$i] = $this->prepare_binary_value($measuring_year, 12);
        $this->explanations[$s][$i][] = '301090 -> 301011 -> 00 4001: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$measuring_year.'</span> (Year, 12 bits)';   
        $i++;   
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($measuring_month, 4);
        $this->explanations[$s][$i][] = '301090 -> 301011 -> 004002: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$measuring_month.'</span> (Month, 4 bits)';   
        $i++;        
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($measuring_day, 6);
        $this->explanations[$s][$i][] = '301090 -> 301011 -> 004003: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$measuring_day.'</span> (Day, 6 bits)';   
        $i++;         
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($measuring_hour, 5);
        $this->explanations[$s][$i][] = '301090 -> 301012 -> 004004: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$measuring_hour.'</span> (Hour, 5 bits)';   
        $i++;         
        
        $this->report_parts[$s][$i] = $this->prepare_binary_value($measuring_minute, 6);
        $this->explanations[$s][$i][] = '301090 -> 301012 -> 004005: <span>'.$this->report_parts[$s][$i].'</span> => <span>'.$measuring_minute.'</span> (Minute, 6 bits)';   
        $i++;     
        ...
        if (isset($this->sensors['rain_in_period']) && $this->sensors['rain_in_period']['sensor_feature_period'])
		{
			...
			else
			{
                ...
                $last_1_hr = $handler_obj->getTotalInLastXHr($this->sensors['rain_in_period']['sensor_feature_id'], $script_runtime, 1, false);
		...
		if (isset($this->sensors['sun_duration_in_period']) && $this->sensors['sun_duration_in_period']['sensor_feature_period'])
		{
			...
			else
			{
                ...
                $last_1_hr = $handler_obj->getTotalInLastXHr($this->sensors['sun_duration_in_period']['sensor_feature_id'], $script_runtime, 1, false);
		...
		{
            $handler_obj = SensorHandler::create($this->sensors['sun_duration_in_period']['handler_id_code']);
			$last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['sun_duration_in_period']['sensor_feature_id'], $measuring_timestamp, 24, false);
        ...
        $this->report_parts[$s][$i] = '100000011000';
        $this->explanations[$s][$i][] = '302043 -> 302040 -> 004024 (Repetition #2): <span>'.$this->report_parts[$s][$i].'</span> => <span>24 * 10^0 - (-2048) = 2072</span> (Time period of precipitation measuring, hrs, 12bits)'; 
        $i++;
        
        if (isset($this->sensors['rain_in_period']))
		{
            $handler_obj = SensorHandler::create($this->sensors['rain_in_period']['handler_id_code']);
            $rain_last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['rain_in_period']['sensor_feature_id'], $measuring_timestamp, 24, false);
            ...
            $expl_val.= '~ round('.$tmp.', 1) * 10^1 -(-1) = '.$tmp_str;
            $expl_val.= '</span>';   
        }
		else
		{
        ...
        $this->explanations[$s][$i][] = '302043 -> 302040 -> 013011 (Repetition #2): <span>'.$this->report_parts[$s][$i].'</span>  => '.$expl_val.' (Total precipitation past 24 hours, kg/m2, 14bits)';             
        $i++;         
        ...
        if (isset($this->sensors['temperature']))
		{
            ...
            $temperature_extr = $handler_obj->getMaxMinInDay($this->sensors['temperature']['sensor_feature_id'], $measuring_timestamp, 1, 0, false);
        }
        ...
        if (isset($this->sensors['temperature']))
		{
            ...
            $expl_val.= ' ~ |round('.$tmp1.', 2) * 10^2 - 0| = '.$tmp. '(+left bit = '.($tmp > 0 ? '0' : '1').' because '.($tmp > 0 ? 'positive' : 'negative').') ';
        } 
		else 
		{
            $this->report_parts[$s][$i] = '1111111111111111';
		...
		// 302043 -> 302041 -> 004024: Time period of temperature measuring
		$this->report_parts[$s][$i] = '100000011000';
        $this->explanations[$s][$i][] = '302043 -> 302041 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>24 - (-2048) = 2072</span> (Time period of temperature measuring, 12bits)';   
        $i++;
		
		// 302043 -> 302041 -> 004024: Time period of temperature measuring
		// 12 bits. Metric = hour. Scale = 0. Ref = -2048
        $this->report_parts[$s][$i] = '111111111111';
        $this->explanations[$s][$i][] = '302043 -> 302041 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>Missing</span> (Time period or displacement, 12bits)';         
        $i++;
        ...
        if (isset($this->sensors['temperature']))
		{
            ...
            $expl_val.= ' ~ |round('.$tmp1.', 2) * 10^2 - 0| = '.abs($tmp).' (+left bit = '.($tmp > 0 ? '0' : '1').' because '.($tmp > 0 ? 'positive' : 'negative').')';
        }
		else
		{
            $this->report_parts[$s][$i] = '1111111111111111';
        ...
        $this->report_parts[$s][$i] = '0001';
        $this->explanations[$s][$i][] = '302043 -> 302042 -> 002002: <span>'.$this->report_parts[$s][$i].'</span> => <span>1</span> (Type of instrumentation for wind measurement, 4 bits)';   
        $i++; 
        ...
        if (isset($this->sensors['wind_direction_1']))
		{
            if ($this->sensors['wind_direction_1']['is_m'])
			{
                $this->report_parts[$s][$i] = '111111111';
			...
			{
                $this->report_parts[$s][$i] = $this->prepare_binary_value($this->sensors['wind_direction_1']['sensor_feature_value'], 9);
                $expl_val = $this->sensors['wind_direction_1']['sensor_feature_value'];   
            }
        }
		else
		{
        ...
        if (isset($this->sensors['wind_speed_1']))
		{
            if ($this->sensors['wind_speed_1']['is_m'])
			{
                $this->report_parts[$s][$i] = '111111111111';
			...
			{
                $tmp1 = It::convertMetric($this->sensors['wind_speed_1']['sensor_feature_value'], $this->sensors['wind_speed_1']['metric_code'], 'meter_per_second');
                $tmp = round($tmp1,1)*10;
                
				$this->report_parts[$s][$i] = $this->prepare_binary_value($tmp, 12);

                $expl_val = $this->sensors['wind_speed_1']['sensor_feature_value'].' '.$this->sensors['wind_speed_1']['metric_code'];
                
				if ($this->sensors['wind_speed_1']['metric_code'] != 'meter_per_second')
				{
                    $expl_val.= ' = '.$tmp1.' meter_per_second';
                }
				
                $expl_val.= ' ~ round('.$tmp1.', 1) * 10^1 = '.$tmp;
            }
        }
		else
		{
        ...
        $this->report_parts[$s][$i] = '100000000001';
        $this->explanations[$s][$i][] = '302045 -> 004024 (Repetition #1): <span>'.$this->report_parts[$s][$i].'</span> => <span>1-(-2048) = 2049</span> (Time period or displacement, Hour, 12 bits)';   
        $i++;
        ...
        if (isset($this->sensors['solar_radiation_in_period']) && $this->sensors['solar_radiation_in_period']['sensor_feature_period'])
		{
			...
			{
                $tmp1 = It::convertMetric($this->sensors['solar_radiation_in_period']['sensor_feature_value'], $this->sensors['solar_radiation_in_period']['metric_code'], 'joule_per_sq_meter');
                $tmp2 = (60/$this->sensors['solar_radiation_in_period']['sensor_feature_period']) * $tmp1;
                $tmp = round($tmp2/100);            

                $str_tmp = $this->prepare_binary_value($tmp, 20);

                $this->report_parts[$s][$i] = $str_tmp;
                $expl_val = $this->sensors['solar_radiation_in_period']['sensor_feature_value'].' '.$this->sensors['solar_radiation_in_period']['metric_code']. ' (during '.$this->sensors['solar_radiation_in_period']['sensor_feature_period'].' min)';
                
				if ($this->sensors['solar_radiation_in_period']['metric_code'] != 'joule_per_sq_meter')
				{
                    $expl_val.= ' = '.$tmp1.' joule_per_sq_meter (during '.$this->sensors['solar_radiation_in_period']['sensor_feature_period'].' min)'; 
                }
				
                if ($this->sensors['solar_radiation_in_period']['sensor_feature_period'] != '60')
				{
                    $expl_val.= ' = (60/'.$this->sensors['solar_radiation_in_period']['sensor_feature_period'].') * '.$tmp1.' (in 60 min) = ' . $tmp2 . ' (in 60 min)';
                }
				
                $expl_val.= ' ~ round('.$tmp2.' * 10^(-2)) = '.$tmp;
            }
        }
		else
		{
        ...
        $this->report_parts[$s][$i] = '100000011000';
        $this->explanations[$s][$i][] = '302045 -> 004024 (Repetition #2): <span>'.$this->report_parts[$s][$i].'</span> => <span>24 - (-2048) = 2072</span> (Time period or displacement, Hour, 12 bits)';   
        $i++;
        ...
        if (isset($this->sensors['solar_radiation_in_period']))
		{
            ...
            $last_24_hr = $handler_obj->getTotalInLastXHr($this->sensors['solar_radiation_in_period']['sensor_feature_id'], $measuring_timestamp, 24, false);
        ...
        $this->report_parts[$s][$i] = '100000011000';
        $this->explanations[$s][$i][] = '302046 -> 004024: <span>'.$this->report_parts[$s][$i].'</span> => <span>24-(-2048) = 2072</span> (Time period or displacement, Hour, 12 bits)';   
        $i++; 
        ...
        //It::debug("BUFR. temperature now: ".$this->sensors['temperature']['sensor_feature_value'].' '.$this->sensors['temperature']['metric_code']);
        //It::debug("BUFR. temperature 24hr ago: ".$sensors_24hr_ago['temperature']['sensor_feature_value'].' '.$sensors_24hr_ago['temperature']['metric_code']);
        
		if (isset($this->sensors['temperature']) && $sensors_24hr_ago['temperature'])
		{
            if ($this->sensors['temperature']['is_m'])
			{
                $this->report_parts[$s][$i] = '111111';
