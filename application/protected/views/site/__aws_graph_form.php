...
$this->widget('TwoDatesFilter', array('block_path' => '#filterparams', 'date_from_name' => 'AWSGraphForm[date_from]', 'date_to_name' => 'AWSGraphForm[date_to]'));
...
<div class="middlewide">
    <div class="middlenarrow">
        ...
        <?php echo CHtml::beginForm($this->createUrl('site/awsgraph'), 'post'); ?>
                        ...
                        <div class="select-list">
                            ...
                            <div class="select-option" id="station-select-list" style="display: none;">
                                <?php echo CHtml::activeCheckBoxList(
                                    ...
                                    array('separator'=>'','labelOptions'=>array(),'template'=>'<li>{beginLabel}{input} {label}</li>', 'container' => 'ul class="select-ul column-4"', 'checkAll' => 'Select All')
                                ) ?>
                            </div>
                        </div>
                    <?php } else {?>
                                            ...
                                            <li  class="head-colors">
                                                ...
                                                if (count($group['stations']) )
                                                    foreach ($group['stations'] as $station ) {
                                                        ?>
                                                        <div class="<?=$station['station_id']?>-station" style="background-color:<?=$station['color']?>; width:5px; height:5px; float:left; margin-left:2px; "></div>
                                                        <?php
                                                    }
                                                ?>
                                            </li>
